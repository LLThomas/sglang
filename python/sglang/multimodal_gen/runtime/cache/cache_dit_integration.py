...
def enable_cache_on_hunyuan_image3(
    transformer: torch.nn.Module,
    config: CacheDitConfig,
    sp_group: Optional[torch.distributed.ProcessGroup] = None,
    tp_group: Optional[torch.distributed.ProcessGroup] = None,
) -> torch.nn.Module:
    """Enable cache-dit on HunyuanImage3DiT.

    HunyuanImage3 is a single-stream GPT-like transformer whose decoder layers
    live in ``self.layers`` and follow ForwardPattern.Pattern_3 (single-stream
    hidden_states in/out).  This differs from the registered ``HunyuanImage``
    adapter which targets HunyuanImage 2.1's dual-stream
    ``transformer_blocks`` / ``single_transformer_blocks`` with Pattern_0.
    Reference: vllm-omni's ``enable_cache_for_hunyuan_image3``.
    """
    if not config.enabled:
        return transformer
    ...
    if config.num_inference_steps is None:
        raise ValueError(
            "num_inference_steps is required for cache-dit. "
            "Please provide it in CacheDitConfig."
        )
    ...
    logger.info(
        "Enabling cache-dit on HunyuanImage3DiT (steps=%d, Fn=%d, Bn=%d, "
        "rdt=%.3f, MC=%d, layers=%d)",
        config.num_inference_steps,
        config.Fn_compute_blocks,
        config.Bn_compute_blocks,
        config.residual_diff_threshold,
        config.max_continuous_cached_steps,
        len(transformer.layers),
    )
